...
spec("hokusai-pocket") do |config|
  ...
  task "mruby" do |args|
    ...
    def build
      ...
      command("unset LD && unset CC && unset CXX && unset AR && rake MRUBY_CONFIG=cli_build_config.rb", chdir: "vendor/mruby")
    ...
    def build
      ...
      ruby do
        klass = Class.new(Hokusai::Block) do
          ...
          attr_accessor :done, :init, :redirect_url
        end

        tmp = klass.mount
        tmp.fetch("https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-#{model}.bin", {method: "GET"}) do |res|
          self.redirect_url = res.body.all.gsub("Found. Redirecting to ", "")
          self.init = true
        end

        while !tmp.init
          Hokusai.worker.run(2)
        end

        i = 0
        host = ""
        path = ""
        parts = tmp.redirect_url.split("")
        parts.each do |part|
          if part == '/'
            i += 1
          end
        ...
        tmp.fetch(host, {method: "GET"}, path: path) do |res|
          File.open("assets/models/ggml-tiny.bin", "w") do |io|
            res.body.on_read do |chunk|
              io.print chunk
            end
          end
