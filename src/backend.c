...
mrb_value on_draw_texture(mrb_state* mrb, mrb_value self)
{
  ...
  Rectangle source = (Rectangle){ 0, 0, (float)wrapper->texture.texture.width, -(float)wrapper->texture.texture.height };
  Rectangle dest = (Rectangle){ x, y, (float)wrapper->texture.texture.width, (float)wrapper->texture.texture.height};
