...
void hp_process_input(mrb_state* mrb, mrb_value input)
{
  ...
  for (int i=0; i<110; i++)
  {
    ...
    // mrb_hash_set(mrb, key, mrb_symbol_value(mrb_intern_lit(mrb, "down")), mrb_bool_value(IsKeyDown(keys[i])));
    // mrb_hash_set(mrb, key, mrb_symbol_value(mrb_intern_lit(mrb, "pressed")), mrb_bool_value(IsKeyPressed(keys[i])));
    // mrb_hash_set(mrb, key, mrb_symbol_value(mrb_intern_lit(mrb, "up")), mrb_bool_value(IsKeyUp(keys[i])));
    // mrb_hash_set(mrb, key, mrb_symbol_value(mrb_intern_lit(mrb, "released")), mrb_bool_value(IsKeyReleased(keys[i])));
  }
...
int hp_backend_run_str(mrb_state* mrb, char* code)
{
  ...
  // get app
  mrb_value backend = mrb_load_string(mrb, code);
  return hp_backend_run(mrb, hokusai_module, backend);
}
...
int hp_backend_run_irep(mrb_state* mrb, uint8_t* code)
{
  ...
  hp_backend_render_callbacks(mrb, hokusai_module);
  // get app
  mrb_value backend = mrb_load_irep(mrb, code);
  if (mrb->exc) mrb_print_error(mrb);
